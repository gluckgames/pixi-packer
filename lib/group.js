...
module.exports = class Group {
    ...
    calculateHash(sprites) {
        ...
        hash.update(this.id || "");
        hash.update(this.variation || "");
        hash.update(this.jpeg ? "y" : "n");
        hash.update(this.quality ? this.quality + "" : "");
        hash.update(this.compressor && this.compressor.constructor ? this.compressor.constructor.name : "");
        hash.update(this.loading_stage || "");
        return hash.digest("hex");
    }

    createAndProcessScaledGroups(sprites) {
        this.scaledGroups = _.map(this.config.scales, (scale, scaleName) => {
            ...
            return new ScaledGroup(scaledSprites, this.hash, scaleName, this.groupConfig, this.config, this.cache, this.cachePath, this.imageProcessor);
        });
        return Promise.all(_.invoke(this.scaledGroups, "process"));
    }
    ...
    process() {
        return this.createAndProcessSprites()
        .then(sprites => {
            this.sprites = sprites;
            this.hash = this.calculateHash(sprites);
            return this.createAndProcessScaledGroups(sprites);
        })
        .then(() => {
