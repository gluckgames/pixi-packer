...
const DEFAULT_SPRITESHEET_CONFIG = {
    max_width: 2048,
    max_height: 1024,
...
module.exports = class ScaledGroup {
    ...
    createAndProcessSpritesheets() {
        let config = _.defaults(this.config.spritesheet || {}, DEFAULT_SPRITESHEET_CONFIG);
        let packer = new MultiBinPacker(config.max_width, config.max_height);
        ...
        console.log("%j", this.scaledSprites.map((scaledSprite, i) => {
            return {
                width: scaledSprite.trim ? scaledSprite.trim.width : scaledSprite.width,
                height: scaledSprite.trim ? scaledSprite.trim.height : scaledSprite.height,
                data: this.groupHash + "_" + this.scaleName + "_" + i
            };
        }))

        if (config.oversized_warning) {
            packer.bins.map(bin => {
