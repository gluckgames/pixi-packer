...
module.exports = class Spritesheet {
    ...
    calculateHash() {
        ...
        hash.update(this.bin.rects.map(rect => rect.data.path).sort().join(" ") + "_");
        hash.update(this.getExtension() + "_");
        hash.update((this.groupConfig.quality ? this.groupConfig.quality : "noquality") + "_");
        hash.update(this.groupConfig.compressor && this.groupConfig.compressor.constructor ? this.groupConfig.compressor.constructor.name : "nocompression");
